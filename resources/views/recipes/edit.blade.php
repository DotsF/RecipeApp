    ...
    <form action="{{ route('recipe.update', ['id' => $recipe['id']]) }}" method="POST"
        ...
        <div class="grid grid-cols-2 rounded border border-gray-400 mt-4">
            <div class="col-span-1">
                <!-- レシピ画像 -->
                <img id="preview" class="object-cover aspect-video w-full mrounded-none" src="{{ $recipe['image'] }}"
                    alt="recipe-image">
            ...
            <div class="col-span-1 p-4">
                <!-- レシピ名の入力 -->
                <input type="text" name="title" value="{{ $recipe['title'] }}" placeholder="レシピ名"
                    class="border border-gray-300 p-2 mb-4 w-full rounded">
                <!-- レシピの説明の入力 -->
                ...
                <select name="category" class="border border-gray-300 p-2 mb-4 w-full rounded">
                    ...
                    @foreach ($categories as $c)
                        <option value="{{ $c['id'] }}"
                            {{ $recipe['category'] ?? null == $c['id'] ? 'selected' : '' }}>
                            {{ $c['name'] }}
                        </option>
                    @endforeach
                </select>
        ...
        <div id="steps">
            @foreach ($recipe['steps'] as $i => $os)
                <div class="step flex justify-between items-center mb-2">
                    @include('components.icon.bars-3')
                    <p class="text-center step-number w-16">手順{{ $i + 1 }}</p>
                    <input type="text" name="steps[]" placeholder="手順を入力" value="{{ $os['description'] }}"
